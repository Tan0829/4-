...
//轮播图
	...
	gwc.onclick=function(){
		...
		}else{loginbg.style.display="block";loginbox.style.display="block";}
	}
	//分享
	...
	//收藏
	var iflogin=localStorage.getItem("token");
	var regoodsid=/\?(goods_id=\d*)/;
	var getmygoods=url.match(regoodsid)[1];
	if(iflogin){if(localStorage.getItem(getmygoods)==1){$("#shoucang").children().attr("src","images/yishoucang.png");}}
	$("#shoucang").click(function(){
		var iflogin=localStorage.getItem("token");
		if(iflogin){
			if(localStorage.getItem(getmygoods)!=1){
				localStorage.setItem(getmygoods,1);
				$("#shoucang").children().attr("src","images/yishoucang.png");
				errinfo.innerHTML="收藏成功";
				error.style.display="block";
				loginbg.style.display="block";
				loginbox.style.display="none";
				setTimeout(function(){error.style.display='none';loginbg.style.display="none";},1000);
			}else{
				localStorage.setItem(getmygoods,0);
				$("#shoucang").children().attr("src","images/weishoucang.png");
				errinfo.innerHTML="取消收藏成功";
				error.style.display="block";
				loginbg.style.display="block";
				loginbox.style.display="none";
				setTimeout(function(){error.style.display='none';loginbg.style.display="none";},1000);
